...
def test_bayes(cts, it=1, cutoff1=10, cutoff2=10, net=net7, net_help=net6, m_main=m7, m_help=m6, s_main=s7, s_help=s6, verify_breadth=None):
    ...
    for j in range(it):
        ...
        keys, scores, v = bayesian_key_recovery(
            [cts[0][i], cts[1][i], cts[2][i], cts[3][i]], num_cand=32, num_iter=5, net=net, m=m_main, s=s_main)
        vtmp = np.max(v)
        ...
        if (vtmp > cutoff1):
            ...
            for i2 in l2:
                ...
                keys2, scores2, v2 = bayesian_key_recovery(
                    [c0a, c1a, c0b, c1b], num_cand=32, num_iter=5, m=m6, s=s6, net=net_help)
                vtmp2 = np.max(v2)
                if (vtmp2 > best_val):
                    best_val = vtmp2
...
def test(n, nr=11, num_structures=100, it=500, cutoff1=10.0, cutoff2=10.0, neutral_bits=[20, 21, 22, 14, 15, 23], keyschedule='real', net=net7, net_help=net6, m_main=m7, s_main=s7,  m_help=m6, s_help=s6, verify_breadth=None):
    ...
    for i in range(n):
        ...
        g = np.max(g)
        good[i] = g
        guess, num_used = test_bayes(ct, it=it, cutoff1=cutoff1, cutoff2=cutoff2, net=net, net_help=net_help,
...
arr1, arr2, good = test(20, nr=12, num_structures=500, it=2000, cutoff1=20.0, cutoff2=500, neutral_bits=neutral13,
                        keyschedule='free', net=net8, net_help=net7, m_main=m8, s_main=s8, m_help=m7, s_help=s7, verify_breadth=128)
